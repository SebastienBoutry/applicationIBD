              ...
              shiny::tabPanel(
                ...
                shiny::fluidRow(
                  shiny::mainPanel(DT::dataTableOutput("Donnees2",
                                            width = "100%"), shiny::downloadButton("downloadData2", "Download"))
                )
              )
            )
          )
...
# En mode SQL
    ...
    # query <- "
    # SELECT inventory.inventory_date, station.station_code, station.station_name_commune, station.lon, station.lat, taxon.taxon_name, taxon_identifier.code, inventory_taxon.abundance
    # FROM inventory
    # JOIN station ON inventory.station_id = station.station_id
    # JOIN inventory_taxon ON inventory.inventory_id = inventory_taxon.inventory_id
    # JOIN taxon ON inventory_taxon.taxon_id = taxon.taxon_id
    # JOIN taxon_identifier ON taxon.taxon_id = taxon_identifier.taxon_id
    # ORDER BY inventory.inventory_date, station.station_code;
    # "
    # 
...
# ---------------------------- Fin Pre-traitement des données 
  ...
  output$tab <- DT::renderDataTable({ 
    ...
    DT::datatable(data() %>% dplyr::filter(lubridate::year(DATE) == input$radio) %>%
  ...
  output$radio <- shiny::renderText({ return(paste("RadioButtons :",input$radio)) })
  ...
  mapFiltered <- shiny::reactive({
    ...
    leaflet_data <- data() %>% dplyr::filter(full_name %in% input$taxons) %>%
      dplyr::mutate(annee = as.factor(lubridate::year(DATE))) %>% 
      dplyr::group_by(full_name) %>% dplyr::mutate(label = dplyr::cur_group_id()) %>%
      dplyr::distinct() %>% 
      dplyr::ungroup() %>%
      dplyr::mutate(Location = paste0(CODE_STATION,"_",
    ...
    names(map_tax) %>%
      purrr::walk(function(df) {
        l <<- l %>%
          leaflet::addMarkers(data=map_tax[[df]],
                     lng=~long, lat=~lat,
                     group = df,
                     popup = ~paste("1- Année: ", annee,'<br>',
  ...
  output$Plot1 <- shiny::renderPlot({
    ...
    data() %>% dplyr::filter(full_name %in% input$taxons) %>%
      dplyr::mutate(annee = as.factor(lubridate::year(DATE))) %>%
